 ...
 */
package com.nxttxn.vramel.processor.exceptionpolicy;
...
public class DefaultExceptionPolicyStrategy implements ExceptionPolicyStrategy {
    ...
    private boolean findMatchedExceptionPolicy(Map<ExceptionPolicyKey, OnExceptionDefinition> exceptionPolicies,
        ...
        for (Map.Entry<ExceptionPolicyKey, OnExceptionDefinition> entry : entries) {
            ...
            if (exchange != null && exchange.getUnitOfWork() != null && type.isFlowScoped()) {
                FlowDefinition route = exchange.getUnitOfWork().getFlowContext() != null ? exchange.getUnitOfWork().getFlowContext().getFlow() : null;
                FlowDefinition typeRoute = ProcessorDefinitionHelper.getFlow(type);
