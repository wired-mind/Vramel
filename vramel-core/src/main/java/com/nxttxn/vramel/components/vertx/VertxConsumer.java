...
public class VertxConsumer extends DefaultConsumer {
    ...
    public VertxConsumer(final Endpoint endpoint, final Processor processor) throws Exception {
        ...
        if (vertx.isWorker()) {
            ...
            logger.info("[Vertx Consumer] Worker detected. Creating Proxy: {} -> {}. See Vertx1.3.1-Final DefaultNetClient.java line 59", proxyAddress, address);
        }


        final ConsumerHandler consumerHandler = new ConsumerHandler(address);
        if (this.endpoint.isLocal()) {
            logger.info("[Vertx Consumer] Local flag detected. Creating a local handler instead of a cluster wide eventbus handler.");
            getEventBus().registerLocalHandler(address, consumerHandler);
        } else {
            getEventBus().registerHandler(address, consumerHandler);
        }
    }
    ...
    private class ConsumerHandler implements Handler<Message<byte[]>> {

        private final String address;

        public ConsumerHandler(String address) {
        ...
        public void handle(final Message<byte[]> message) {
            ...
            logger.info("[Vertx Consumer] [{}] Received Message", address);
            ...
            try {
                ...
                logger.error(String.format("[Vertx Consumer] Error processing flow: %s", address), e);
